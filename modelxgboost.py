...
print(f"\nEvaluating XGBoost Regressor for K features from {min(k_values)} to {max(k_values)}...")
...
for k in k_values:
    ...
    pipeline = Pipeline(steps=[
        ...
        ('regressor', xgb_model) # Use XGBoost model here
    ])
    ...
    # --- Prediction Workaround for XGBoost ---
    try:
        # 1. Apply preprocessing and feature selection steps to X_test
        X_test_transformed = pipeline.named_steps['preprocessor'].transform(X_test)
        X_test_selected = pipeline.named_steps['selector'].transform(X_test_transformed)

        # 2. Predict using the fitted regressor directly on the transformed data
        y_pred = pipeline.named_steps['regressor'].predict(X_test_selected)

    except Exception as e:
        print(f"  Error predicting for K={k}: {e}")
        continue
    # --- End of Prediction Workaround ---
...
if not results_k:
    print("\nNo results were generated for XGBoost. Check for errors during processing.")
else:
    ...
    # --- Updated Title ---
    print("\n--- XGBoost Model Performance for Different Numbers of Selected Features (K) ---")
    # -------------------
...
print("and the XGBoost Regressor itself to maximize reproducibility across platforms.")
print("Prediction workaround applied for compatibility within scikit-learn pipeline.")
print("If results still differ slightly, check library versions (scikit-learn, pandas, numpy, xgboost).")