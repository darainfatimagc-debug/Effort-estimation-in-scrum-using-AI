...
print(f"\nEvaluating SVR for K features from {min(k_values)} to {max(k_values)}...")
...
for k in k_values:
    ...
    # Common kernel is 'rbf'. C and gamma are important hyperparameters to tune.
    # Using default values for this example.
    ...
    pipeline = Pipeline(steps=[
        ...
        ('regressor', svr_model) # Use SVR model here
    ])
    ...
    # Make Predictions (Standard predict works fine for SVR)
    try:
...
if not results_k:
    print("\nNo results were generated for SVR. Check for errors during processing.")
else:
    ...
    # --- Updated Title ---
    print("\n--- SVR Model Performance for Different Numbers of Selected Features (K) ---")
    # -------------------
...
print("SVR itself is deterministic given hyperparameters and data.")
