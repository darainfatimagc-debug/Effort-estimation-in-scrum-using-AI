...
print(f"\nEvaluating KNN Regressor for K features from {min(k_values)} to {max(k_values)}...")
...
for k in k_values:
    ...
    # Using n_neighbors=5 as a default, matching the paper's likely approach
    # KNN itself is deterministic, so no random_state needed here
    ...
    pipeline = Pipeline(steps=[
        ...
        ('regressor', knn_model) # Use KNN model here
    ])
    ...
    # Make Predictions (Standard predict works fine for KNN)
    try:
...
if not results_k:
    print("\nNo results were generated for KNN. Check for errors during processing.")
else:
    ...
    # --- Updated Title ---
    print("\n--- KNN Model Performance for Different Numbers of Selected Features (K) ---")
    # -------------------
...
print("KNN itself is deterministic once data and n_neighbors are fixed.")
