...
print(f"\nEvaluating Extra Trees Regressor for K features from {min(k_values)} to {max(k_values)}...")
...
for k in k_values:
    ...
    pipeline = Pipeline(steps=[
        ...
        ('regressor', et_model) # Use Extra Trees model here
    ])
    ...
    # Make Predictions (Standard predict works fine for ExtraTrees)
    try:
...
if not results_k:
    print("\nNo results were generated for Extra Trees. Check for errors during processing.")
else:
    ...
    # --- Updated Title ---
    print("\n--- Extra Trees Model Performance for Different Numbers of Selected Features (K) ---")
    # -------------------
...
print("and the Extra Trees Regressor itself to maximize reproducibility across platforms.")
