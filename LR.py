...
numeric_transformer = Pipeline(steps=[
    ...
    ('scaler', StandardScaler()) # Scaling often helps LR, though not strictly required like SVM/KNN
])
...
print(f"\nEvaluating Linear Regression for K features from {min(k_values)} to {max(k_values)}...")
...
for k in k_values:
    ...
    # No random_state needed as it's deterministic
    ...
    pipeline = Pipeline(steps=[
        ...
        ('regressor', lr_model) # Use Linear Regression model here
    ])
    ...
    # Make Predictions (Standard predict works fine for LinearRegression)
    try:
...
if not results_k:
    print("\nNo results were generated for Linear Regression. Check for errors during processing.")
else:
    ...
    # --- Updated Title ---
    print("\n--- Linear Regression Model Performance for Different Numbers of Selected Features (K) ---")
    # -------------------
    print(results_df.round(2)) # Rounding to 2 decimal places
    print("--------------------------------------------------------------------------------------")

# --- Add note about reproducibility ---
print("\nNote: Random states have been set for train/test split and mutual information scoring.")
print("Linear Regression itself is deterministic given the data.")
print("If results differ slightly across platforms, check library versions (scikit-learn, pandas, numpy).")